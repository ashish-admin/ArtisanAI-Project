...
expected in the Authorization header as a Bearer token.
The tokenUrl points to the endpoint where clients can get a token.
...
"""
A dependency that yields a new database session per request.
This is an alternative way to define the get_db dependency if we want to
keep it within the API layer's dependency module.
"""
try:
db = SessionLocal()
yield db
